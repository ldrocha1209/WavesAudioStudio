# Waves v1.0.2 Local Release Results
...
## v1.0.2 YouTube playback correction

- The downloaded YouTube processing source is commonly Opus in a WebM container, which macOS WebKit did not reliably play through Tauri's local asset protocol.
- Waves now keeps that original-quality source for processing and creates a separate 192 kbps MP3 preview for the second-screen player.
- The player preloads the preview, reports readiness, and shows “Pre-processing playback is not available for this YouTube link” if WebKit rejects a particular preview instead of presenting a silent control.
- A fresh-cache smoke through the frozen production engine produced both `source.webm` and a distinct `preview.mp3`; macOS `afinfo` validated the preview as 48 kHz stereo MP3 audio.
- yt-dlp progress and diagnostic output is suppressed so the downloader cannot corrupt the engine's stdout JSONL protocol during a first-time download.
...
- All 23 deterministic Python tests pass, covering protocol bounds/versioning, engine process behavior, monotonic and terminal job invariants, media inspection and source immutability, macOS-compatible YouTube preview generation, downloader protocol isolation, export formats/collision naming, YouTube URL/playlist handling, multi-output selection and validation, one-inference stem mapping, CPU policy, and cancellation during model load.
...
- Version metadata is aligned at `1.0.2` across npm, Cargo, Tauri, and the Python engine.
...
- DMG SHA-256: `2da6f1b27b38a412da6bb02cde65ebf3646befc86344a5c4a6a34e992efc9281`.

Local artifact paths (generated, not tracked):

```text
src-tauri/target/release/bundle/macos/Waves.app
src-tauri/target/release/bundle/dmg/Waves_1.0.2_x64.dmg
```
...
- No prebuilt GitHub binary is published for v1.0.2. The artifact is Intel-only, ad-hoc signed, very large, and its Homebrew FFmpeg build is dynamically linked and GPL-enabled. Shipping it as an easy public download would require dependency bundling, architecture/signing work, and a separate compliance review.
...
hdiutil verify src-tauri/target/release/bundle/dmg/Waves_1.0.2_x64.dmg
```
