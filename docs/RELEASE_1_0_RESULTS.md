# Waves v1.0 Local Release Results
...
- A real permitted single-video YouTube smoke test passed through the Waves adapter with the bundled Node runtime and exactly one downloaded audio item.
...
- Version metadata is aligned at `1.0.0` across npm, Cargo, Tauri, and the Python engine.
...
- The frozen engine and the exact engine copied inside the signed `.app` each completed a real CPU Instrumental export through protocol v1.
...
- DMG SHA-256: `a9ac5829ba88a51dcebd38841e1998a6bc2dbbb2882d2e10e678b9b3aefda1c9`.

Local artifact paths (generated, not tracked):

```text
src-tauri/target/release/bundle/macos/Waves.app
src-tauri/target/release/bundle/dmg/Waves_1.0.0_x64.dmg
```
...
- No prebuilt GitHub binary is published for v1.0. The artifact is Intel-only, ad-hoc signed, very large, and its Homebrew FFmpeg build is dynamically linked and GPL-enabled. Shipping it as an easy public download would require dependency bundling, architecture/signing work, and a separate compliance review.
...
hdiutil verify src-tauri/target/release/bundle/dmg/Waves_1.0.0_x64.dmg
```
